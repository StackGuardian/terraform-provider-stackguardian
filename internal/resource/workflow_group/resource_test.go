...
func deleteWorkflowGroupFixture(id string) {
	...
	client.WorkflowGroups.DeleteWorkflowGroup(context.TODO(), config.Get().OrgName, id)
}
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				PreConfig: func() {
					client := acctest.SGClient()
					_, err := client.WorkflowGroups.DeleteWorkflowGroup(context.TODO(), config.Get().OrgName, workflowGroupName)
					if err != nil {
						t.Fatal(err)
					}
				},
...
func TestAccWorkflowGroupOptionalId(t *testing.T) {
	...
	t.Cleanup(func() { deleteWorkflowGroupFixture(wfGrpID) })
