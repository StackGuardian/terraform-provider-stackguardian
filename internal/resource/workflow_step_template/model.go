...
func BuildAPIModelToWorkflowStepTemplateModel(apiResponse *workflowsteptemplate.UpdateWorkflowStepTemplateResponse) (*WorkflowStepTemplateResourceModel, diag.Diagnostics) {
	...
	// Handle context tags
	if apiResponse.ContextTags != nil && len(apiResponse.ContextTags) > 0 {
	...
	if apiResponse.SharedOrgsList != nil && len(apiResponse.SharedOrgsList) > 0 {
	...
	if apiResponse.RuntimeSource != nil {
		...
		if apiResponse.RuntimeSource.AdditionalConfig != nil && len(apiResponse.RuntimeSource.AdditionalConfig) > 0 {
