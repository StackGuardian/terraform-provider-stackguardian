...
func resourceStackGuardianStackAPI() *schema.Resource {
	...
	return &schema.Resource{
		Create: resourceStackGuardianStackAPICreate,
		Read:   resourceStackGuardianStackAPIRead,
		Update: resourceStackGuardianStackAPIUpdate,
		Delete: resourceStackGuardianStackAPIDelete,
		Exists: resourceStackGuardianStackAPIExists,

		Importer: &schema.ResourceImporter{
			State: resourceStackGuardianStackAPIImport,
		},
		...
		Schema: map[string]*schema.Schema{
			...
			"data": {
				Type:        schema.TypeString,
				Description: "Valid JSON data that this provider will manage with the API server. Please refer to the API Docs: https://docs.stackguardian.io/api#tag/Stacks",
				Required:    true,
...
Since there is nothing in the ResourceData structure other
	...
	from the API
*/
func resourceStackGuardianStackAPIImport(d *schema.ResourceData, meta interface{}) (imported []*schema.ResourceData, err error) {
	input := d.Id()
...
func resourceStackGuardianStackAPICreate(d *schema.ResourceData, meta interface{}) error {
	obj, err := make_api_object_stack(d, meta)
	if err != nil {
		return err
	}
...
func resourceStackGuardianStackAPIRead(d *schema.ResourceData, meta interface{}) error {
	obj, err := make_api_object_stack(d, meta)
	if err != nil {
		return err
	}
...
func resourceStackGuardianStackAPIUpdate(d *schema.ResourceData, meta interface{}) error {
	obj, err := make_api_object_stack(d, meta)
	if err != nil {
		return err
	}
...
func resourceStackGuardianStackAPIDelete(d *schema.ResourceData, meta interface{}) error {
	obj, err := make_api_object_stack(d, meta)
	if err != nil {
		return err
	}
...
func resourceStackGuardianStackAPIExists(d *schema.ResourceData, meta interface{}) (exists bool, err error) {
	obj, err := make_api_object_stack(d, meta)
	if err != nil {
		return exists, err
	}
...
*/
func make_api_object_stack(d *schema.ResourceData, meta interface{}) (*api_object, error) {
	opts, err := buildApiObjectStackOpts(d)
	if err != nil {
		return nil, err
	}
...
func buildApiObjectStackOpts(d *schema.ResourceData) (*apiObjectOpts, error) {
