...
<!-- AI-SKILLS:START -->
Generating configuration from this guide? Load the **`stackguardian-templates`** skill, which turns the guidance here into rules an agent can follow.

**Worth knowing either way:** An attribute never written down can change when a revision changes, because it was always coming from the template.
