...
# Documentation lives at the REPO ROOT, not under internal/:
docs/                       # Generated by tfplugindocs. Never edit by hand.
docs-templates/             # Page templates (.md.tmpl) + guides. Edit these.
docs-examples/              # .tf snippets embedded into pages via {{tffile}}
docs-guides-assets/         # Runnable quickstart / onboarding projects
scripts/validate-examples.sh # Type-checks every example (make docs-validate-examples)
```

### Documentation workflow

```bash
make tools-install           # install the pinned tfplugindocs
make docs-generate           # regenerate docs/ from docs-templates/ + schemas
make docs-validate           # structure / frontmatter checks
make docs-check              # fails if docs/ is stale (CI gate)
make docs-validate-examples  # terraform validate on every example (no credentials)
```

Attribute text comes from `MarkdownDescription` in `schema.go`, mostly via shared constants in
`internal/constants/`. A data source should reuse the same constant as its resource twin so the
two cannot drift. Page-level prose goes in the `.md.tmpl`; `{{ .Description }}` renders the
schema-level description into both the frontmatter and the page body.
...
Authoritative list: `Resources()` in `internal/provider/provider.go`.

| Terraform type                                  | `internal/resource/` path         |
| ----------------------------------------------- | --------------------------------- |
| `stackguardian_connector`                       | `connector/`                      |
| `stackguardian_policy`                          | `policy/`                         |
| `stackguardian_role`                            | `role/`                           |
| `stackguardian_rolev4`                          | `role_v4/`                        |
| `stackguardian_role_assignment`                 | `role_assignment/`                |
| `stackguardian_runner_group`                    | `runner_group/`                   |
| `stackguardian_stack_template`                  | `stack_template/`                 |
| `stackguardian_stack_template_revision`         | `stack_template_revision/`        |
| `stackguardian_workflow_from_template`          | `workflow_from_template/`         |
| `stackguardian_workflow_git`                    | `workflow_git/`                   |
| `stackguardian_workflow_group`                  | `workflow_group/`                 |
| `stackguardian_workflow_step_template`          | `workflow_step_template/`         |
| `stackguardian_workflow_step_template_revision` | `workflow_step_template_revision/`|
| `stackguardian_workflow_template`               | `workflow_template/`              |
| `stackguardian_workflow_template_revision`      | `workflow_template_revision/`     |

Note the directory `role_v4/` produces the Terraform type `stackguardian_rolev4`.
