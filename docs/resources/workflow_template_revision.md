...
page_title: "stackguardian_workflow_template_revision Resource - stackguardian"
subcategory: ""
description: |-
  Manages a workflow template revision resource.
---
...
- `kind` (String) Deployment platform kind (AWS_STATIC, AWS_RBAC, AWS_OIDC, AZURE_STATIC, AZURE_OIDC, GCP_STATIC, GCP_OIDC).
...
- `kind` (String) Kind of the environment variable (TEXT, SECRET_REF).
...
- `secret_id` (String) ID of the secret (if using vault secret). Use if type is SECRET_REF
- `text_value` (String) Text value (if using plain text). Use if type is TEXT
...
- `encoded_data` (String) Encoded schema data.
- `type` (String) Type of the schema.
- `ui_schema_data` (String) UI schema data (JSON).
...
- `webhooks` (Attributes) Configuration for webhooks to be triggered on complemention. Statuses on which webhooks can be sent (approval_required, cancelled, completed, drift_detected, errored) (see [below for nested schema](#nestedatt--mini_steps--webhooks))
- `wf_chaining` (Attributes) Configuration for other workflows to be triggered on complemention. Statuses on which webhooks can be sent (completed, errored) (see [below for nested schema](#nestedatt--mini_steps--wf_chaining))
...
- `email` (Attributes) Configuration for email notifications to be sent on complemention. Statuses on which notifications can be sent (approval_required, cancelled, completed, drift_detected, errored) (see [below for nested schema](#nestedatt--mini_steps--notifications--email))
...
- `state` (String) State of the schedule (ENABLED, DISABLED).
