...
- `auth` (String, Sensitive) Credential for cloning the repository, as a path-form ID. Required whenever `is_private` is `true`, and required unconditionally (regardless of `is_private`) for every `source_config_dest_kind` except `GIT_OTHER` — only `GIT_OTHER` may omit it, for a public repository. When set: `GIT_OTHER` requires a secret, `/secrets/<secret-name>`; every other kind requires a VCS connector, `/integrations/<connector-name>`, built as `"/integrations/${stackguardian_connector.github.id}"`.
